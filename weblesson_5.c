...
enum {MAX_TAIL_SIZE=100, START_TAIL_SIZE=20, MAX_FOOD_SIZE=20, FOOD_EXPIRE_SECONDS=10,SEED_NUMBER=10};
...
void update(struct snake_t *head, struct food f[], int key, int ai)
{
    if (ai)
...
snake_t* snakes[PLAYERS];
    ...
    int key_pressed=0;
    int exit_flag = 0;
    while( key_pressed != STOP_GAME && !exit_flag )
    {
        clock_t begin = clock();
